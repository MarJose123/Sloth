...
export default function EditAccountScreen() {
    ...
    (async () => {
      try {
        ...
        if (account.logoKey && account.logoKey.startsWith("bank/")) {
          setBadgeMode("logo");
          setSelectedLogoKey(account.logoKey);
        } else if (account.logoKey && account.logoKey.startsWith("custom/")) {
          setBadgeMode("custom");
          const filename = account.logoKey.slice("custom/".length);
  ...
  const handleSave = handleSubmit(onSubmit, (fieldErrors) => {
    ...
    if (entry) {
      const [, error] = entry;
      toast.warning("Name", { description: error.message as string });
    }
  });
